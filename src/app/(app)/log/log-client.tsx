...
export function LogClient({
  ...
  const [pendingResult, setPendingResult] =
    useState<MealAnalysisResponse | null>(null);
  ...
  const handlePhotoCapture = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (ev) => {
      ...
      try {
        ...
        const data: MealAnalysisResponse & { error?: string } =
          await res.json();
    ...
    <div className="flex flex-col">
        ...
        <Tabs defaultValue="camera">
          ...
          <TabsContent value="camera" className="mt-4 space-y-4">
            ...
            {todayMeals.length > 0 && (
              <Section
                title="今日の記録"
                description={`${todayMeals.length}品`}
              >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        ...
                        {" · "}
                        {new Date(meal.logged_at).toLocaleDateString("ja-JP", {
                          month: "numeric",
                          day: "numeric",
                        })}
                      </p>
      ...
      {editingMeal && (
        <MealEditDialog
          ...
          onSaved={(updated) => {
            setTodayMeals(
              todayMeals.map((m) => (m.id === updated.id ? updated : m)),
            );
            setEditingMeal(null);
          }}
        />
