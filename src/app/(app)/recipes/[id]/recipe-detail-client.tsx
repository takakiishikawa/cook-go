...
import { ArrowLeft, ShoppingCart, Clock, Check, X } from "lucide-react";
  ...
  const generateShoppingList = async () => {
    ...
    try {
      ...
      if (data.items?.length === 0) {
        toast.success("すべての食材がストックにあります！");
      } else {
    ...
    <div className="flex flex-col">
      <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-sm border-b border-border">
        <div className="flex items-center gap-3 px-4 py-3">
          <Link href="/recipes" className="p-2 rounded-xl hover:bg-muted">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <h1 className="font-heading text-lg flex-1 truncate">{recipe.title}</h1>
        </div>
      </div>
      ...
      <div className="px-4 pt-4 space-y-6 pb-6">
        <div className="flex flex-wrap gap-3">
          {recipe.protein_g_per_serving && (
            <div className="bg-primary/10 rounded-xl px-4 py-2 text-center">
              ...
              <p className="font-semibold text-primary text-lg">{recipe.protein_g_per_serving}g</p>
            </div>
          )}
          {recipe.calorie_kcal_per_serving && (
            <div className="bg-muted rounded-xl px-4 py-2 text-center">
              ...
              <p className="font-semibold text-lg">{recipe.calorie_kcal_per_serving}kcal</p>
            </div>
          )}
          {recipe.prep_time_min && (
            <div className="bg-muted rounded-xl px-4 py-2 text-center flex items-center gap-2">
              <Clock className="w-4 h-4 text-muted-foreground" />
              <div>
                <p className="text-xs text-muted-foreground">調理時間</p>
                <p className="font-semibold">{recipe.prep_time_min}分</p>
              </div>
            </div>
          )}
          {recipe.servings && recipe.servings > 1 && (
            <div className="bg-muted rounded-xl px-4 py-2 text-center">
              ...
              <p className="font-semibold text-lg">{recipe.servings}食分</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="font-heading text-lg">食材リスト</h2>
            <span className="text-xs text-muted-foreground">
              ストック: {inPantryCount} / 買う必要: {needToBuyCount}
            </span>
          </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn("text-sm", inPantry ? "text-foreground" : "text-foreground")}>{ing.name}</span>
                    </div>
        ...
        {steps.length > 0 && (
          <div className="space-y-3">
            <h2 className="font-heading text-lg">作り方</h2>
            <div className="space-y-3">
              {steps.map((step) => {
